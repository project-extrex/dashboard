...
$router->get('/login', function () use ($renderer) {
    ...
    $renderer->render('login', ['login_path' => '/login']);
});
...
$router->post('/login', function () use ($renderer, $entityManager) {
    ...
    if (!$user) {
        ...
        return wantsJson()
            ...
            : $renderer->render('login', ['login_path' => '/login', 'error' => $errorMsg]);
    }
...
$router->post('/register', function () use ($entityManager, $renderer, $settingsRepo) {
    ...
    if (!$username || !$firstname || !$lastname || !$email || !$password) {
        return wantsJson()
            ...
            : $renderer->render('register', ['error' => 'All fields are required.']);
    }

    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        return wantsJson()
            ? jsonResponse(['success' => false, 'error' => 'Invalid email address'], 400)
            : $renderer->render('register', ['error' => 'Invalid email address.']);
    }

    if ($password !== $confirmpass) {
        return wantsJson()
            ? jsonResponse(['success' => false, 'error' => 'Passwords do not match'], 400)
            : $renderer->render('register', ['error' => 'Passwords do not match.']);
    }

    if (strlen($password) < 8) {
        return wantsJson()
            ? jsonResponse(['success' => false, 'error' => 'Password must be at least 8 characters'], 400)
            : $renderer->render('register', ['error' => 'Password must be at least 8 characters.']);
    }
    ...
    if ($userRepo->findOneBy(['email' => $email])) {
        return wantsJson()
            ...
            : $renderer->render('register', ['error' => 'Email already registered.']);
    }

    if ($userRepo->findOneBy(['username' => $username])) {
        return wantsJson()
            ? jsonResponse(['success' => false, 'error' => 'Username already taken'], 400)
            : $renderer->render('register', ['error' => 'Username already taken.']);
    }
