...
use App\Database\Entities\Servers; // if you have a Server entity
global $entityManager;

// Count users
$totalUsers = (int) $entityManager->createQuery(
    'SELECT COUNT(u.id) FROM ' . User::class . ' u'
)->getSingleScalarResult();

// Count servers (replace with your actual entity)
$totalServers = (int) $entityManager->createQuery(
    'SELECT COUNT(s.id) FROM ' . Servers::class . ' s'
)->getSingleScalarResult();
?>
...
<div class="min-h-screen bg-white text-gray-800 p-6">
  ...
  <div class="mb-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="bg-gray-50 p-4 rounded-2xl shadow">
        ...
        <p class="text-2xl font-bold mt-2"><?= htmlspecialchars($totalUsers) ?></p>
      </div>
      <div class="bg-gray-50 p-4 rounded-2xl shadow">
        <h3 class="text-lg font-medium flex items-center gap-2">
        ...
        <p class="text-2xl font-bold mt-2"><?= htmlspecialchars($totalServers) ?></p>
      </div>
    </div>
  </div>
  ...
  <div class="text-sm text-gray-500 flex items-center gap-2">
    ...
    Your dashboard is up to date <span class="font-semibold">(v1.0.0)</span>
  </div>
</div>